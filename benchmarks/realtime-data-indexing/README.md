...
few seconds: `VmRSS` for the time series, `VmHWM` (kernel-tracked high-water
...
Outputs land in `results/`: `<size>-memory.png` (RSS over time),
`<size>-memory.csv`, `<size>-summary.json` (duration, MB/min, chunk count,
peak RSS, retrieval accuracy).
