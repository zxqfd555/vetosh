"""Interactive terminal wizard: ``serviette quickstart``.

Generates a serviette YAML config by asking a short series of questions. In a real
...
def _vector_db_section(answers: dict[str, Any]) -> dict[str, Any]:
    ...
    if vdb_type == "weaviate":
        section = {
            ...
            "collection": answers.get("weaviate_collection", "ServietteEmbeddings"),
        }
    ...
    def run(self) -> dict[str, Any]:
        ...
        if vdb_type == "duckdb":
            ...
            answers["weaviate_collection"] = self.p.text(
                "Weaviate collection (capitalized)", default="ServietteEmbeddings"
            )
